...
```ts
Given('I am working as a/an {string}', async ({ shell }, role) => { ... });
```

The same applies to any word the sentence inflects: `is/are`, `it/they`. Catching this at the
step definition is much cheaper than discovering it as a "missing step definition" error after
a scenario is already written.
